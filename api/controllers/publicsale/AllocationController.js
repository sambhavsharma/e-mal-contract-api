...
module.exports = {
    ...
    get_total_allocated_tokens: function(req, res){
        total_allocated_tokens = Web3Service.eMalPublicsale.totalTokensAllocated();
    ...
    get_allocated_tokens: function(req, res){
        allocated_tokens = Web3Service.eMalPublicsale.getAllocatedTokens(req.param('investor_addr'));
        return res.status(200).json({status: 'success', 
    ...
    allocate: function(req, res){
        Web3Service.eMalPublicsale.allocateTokens.sendTransaction(req.param("investor_addr"), 
    ...
    deduct: function(req, res){
        Web3Service.eMalPublicsale.deductAllocatedTokens.sendTransaction(req.param("investor_addr"), 
                Web3Service.web3.toWei(req.param("tokens")));
        return res.status(200).json({status: 'success' });
    }
}